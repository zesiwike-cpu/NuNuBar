...
air65-v3-macos-wired 或 air96-v2-ansi-macos-v7；否则必须停下来如实说明。
在你说明当前这一个具体操作，
...
Air65 V3 不需要 VIA 备份、恢复镜像或任何固件材料。Air96 V2 也必须先测试
现有固件；只有灯光自检失败后，才准备 VIA JSON 备份和准确官方恢复固件，且两者
都保存在仓库之外，不提交到 Git 或公开 Issue。

Air65 V3 状态灯不依赖 Karabiner。可选的按键和旋钮映射编辑器目前使用官方
Karabiner-Elements 执行准确设备限定规则。请按
[`AIR65_V3_KEY_MAPPING.zh-CN.md`](AIR65_V3_KEY_MAPPING.zh-CN.md) 执行；安装
...
这两款是仅有的普通用户路线。Air60 V2、Air75 V2、Halo75 V2、Windows 和其他
...
依赖 USB Raw HID 以及匹配的 NuNuBar 固件；Air65 V3 直接使用官方有线控制接口。
Air65 V3 可以通过蓝牙打字，但当前版本的 Codex 状态灯必须使用 USB 有线模式。
连接 Codex 是另一项配置变更：
...
只有准确的 Air96 V2 ANSI 在现有固件自检失败后才需考虑固件。Air65 V3 已有
官方接口，禁止进入 DFU 或刷写 V2 固件。先阅读
...
Air96 V2 在固件中渲染灯效；Air65 V3 使用官方控制接口，其中“闪烁”由 App
生成。两条已验证路线都使用 USB。字节级协议见 `docs/NBAR_PROTOCOL.md`。
