...
Codex 会先运行只读预检，只能选择下面三条已验证 macOS 路线之一。其他电脑或
...
NuNuBar 当前有三条实机成功的普通用户路线：

1. Apple Silicon macOS + Air65 V3 + 有线 USB，直接使用官方接口，完全不刷
   固件；Codex 状态灯和可选的右旋任务切换已实机验证。
2. Apple Silicon macOS + Air75 V3 + 有线 USB，使用官方 `1.0.14.6` 或更高
   固件；旧版本会被 App 检出，并在单独确认后通过 NuPhyIO 备份和官方升级。
3. Apple Silicon macOS + Air96 V2 ANSI + 有线 USB；先测试现有固件，只有自检
   失败时才进入已验证的 v7 固件流程。
...
| 系统 | 客户端 | 连接方式 | 普通用户状态 |
|---|---|---|---|
| macOS 14+，Apple Silicon | 原生菜单栏 App 和首次设置向导 | Air65/Air75 V3 官方有线 HID 或 Air96 V2 Raw HID | 已实机验证 |
...
| Air75 V3 | `19F5:1028` | 侧灯 | 官方 `1.0.14.6` 或更高；官网青色、NuNuBar 各状态和真实 Codex 工作到完成切换已实机验证 |
...
Air75 V3 使用同类 64 字节官方有线接口，但有独立的 `D6` 侧灯布局和最低固件
要求，详见 [Air75 V3 验证记录](docs/AIR75_V3_VERIFICATION.zh-CN.md)。
如需待机灯始终常亮，短按 `Fn + ]` 关闭键盘固件自己的自动休眠，再在 NuNuBar
中选择非黑色待机颜色和“常亮”。这是官方固件开关，不需要 HID 轮询保活。
在 macOS 上，可以分别使用 [Air65 V3](docs/AIR65_V3_KEY_MAPPING.zh-CN.md) 和
[Air75 V3](docs/AIR75_V3_KEY_MAPPING.zh-CN.md) 的型号独立映射编辑器，点选准确
实体配列并创建设备限定的系统或 Codex 动作，也可分别映射 NuPhyIO 的
`F21`/`F22`/`F23` 旋钮输入；此前已验证的 Air65 黄色 `PGDN` 到 Fn/地球键路线
无需迁移，继续兼容。

> [!NOTE]
> 状态灯同步**不需要** Karabiner。Air65/Air75 V3 当前的按键和旋钮映射需要 macOS
